...
understands the intended behaviour can catch them (OWASP WSTG Business Logic
Testing; CWE-840).

## Two axes: does it match the spec, and does it follow the code's own rules?

Review against the **intent summary** the spine established (commits, PR body,
ticket, plan, ADR). Report on both axes and keep them separate — a change can
pass one and fail the other:

- **Spec axis** — requirements the intent asked for that are missing or
  partial; behaviour in the diff that was *not* asked for (scope creep — an
  out-of-scope regression hides in the part of the diff nobody was looking
  at); requirements that look implemented but whose implementation is wrong.
  Quote the intent line for each.
- **Code axis** — does the change do what the surrounding names, comments,
  types, and call sites say it should, at *every* consumer.

## Trace a concrete input — the single most productive check

Pick a real scenario (actual values, actual state) and follow it through the
changed path step by step: the value at each line, the branch taken, the state
after each call. Most correctness bugs surface the moment you compute a real
case instead of describing the logic. Do it for the happy path and for one
boundary (empty / one / max / undefined / error).

## Sentinels that now mean two things

When a change adds a return path that reuses an existing sentinel (`null`,
`undefined`, `[]`, `{}`, a fallback enum, `0`), audit consumers for *semantic*
handling, not just type acceptance. If `null` now means both "not loaded yet"
and "no result", the empty-state UI, the metric, or the retry logic downstream
is wrong even though nothing crashes. Require a richer return shape or an
explicit state that preserves the distinction.
