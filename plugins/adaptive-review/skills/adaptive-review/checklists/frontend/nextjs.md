...
`typescript.md`; load those too for `.tsx` files. Server-side handler logic is
judged with `core/security.md` and `core/intent-and-correctness.md` (a backend
checklist can be added under `checklists/backend/` later).
...
- Redirects/rewrites validate their targets (no open redirect — `core/security.md`).
